...
class TestEdgeIntegration:
    ...
    def test_edge_not_found_without_db(self, mock_chrome_path):
        """Test that Edge raises error when no database is available."""
        with pytest.raises(BrowserNotFoundError):
            HistoryService.search_history(query="test", limit=10, browser="edge")
